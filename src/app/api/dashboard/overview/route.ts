...
export async function GET(req: Request) {
  ...
  try {
    ...
    const tradeWhere = agentIds.length > 0 ? { agentId: { in: agentIds } } : {};

    const [recentTrades, blockEvents, totalTradeCount, totalPnl] = await Promise.all([
      db.trade.findMany({
        where: tradeWhere,
        orderBy: { createdAt: 'desc' },
        take: 20,
      }),
      db.blockEvent.findMany({
        where: agentIds.length > 0 ? { agentId: { in: agentIds } } : {},
        orderBy: { createdAt: 'desc' },
        take: 10,
      }),
      db.trade.count({ where: tradeWhere }),
      db.trade.aggregate({
        _sum: { pnlDelta: true },
        where: {
          status: 'executed',
